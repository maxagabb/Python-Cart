...
def show_menu():
    ...
    print("cat  - show items by category")
...
def show_categories(categories):
    ...
    line_format1 = "{:<5s} {:<25s}"
    line_format2 = "{:<5d} {:<25s}"
    print(line_format1.format("ID", "Name"))
    for i in range(len(categories)):
        category = categories[i]
        print(line_format2.format(
            category.ID,
            category.name))
    print()
...
def show_by_cat():
    ...
    show_products(db.get_products_by_category(id))

def main():
    show_title()
    show_menu()
    # create database ==|> in db.py
